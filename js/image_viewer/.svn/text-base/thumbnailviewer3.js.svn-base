...
jQuery.thumbnailviewer2={
		loadmsg: '<center><img src="../../images/spinningred.gif" /><p style="font-size:11px; color:#057dd7; text-align:center;">Loading Large Image...</p></center>', //HTML for loading message. Make sure image paths are correct

	/////NO NEED TO EDIT BEYOND HERE////////////////

	dsetting: {trigger:'mouseover', preload:'yes', fx:'fade', fxduration:500, enabletitle:'yes'}, //default settings
	buildimage:function($, $anchor, setting){
		var imghtml='<img src="'+$anchor.attr('href')+'" style="border-width:0" width="170" height="170" />'
		if (setting.link)
			imghtml='<a href="'+setting.link+'">'+imghtml+'</a>'
		imghtml=imghtml+((setting.enabletitle!='no' && $anchor.attr('title')!='')? '<br />'+$anchor.attr('title') : '')
		return $(imghtml)
	},
...
jQuery.fn.addthumbnailviewer2=function(options){
	...
	return this.each(function(){ //return jQuery obj
		...
		$anchor.unbind(triggerevt).bind(triggerevt, function(){
			if ($loadarea_new.data('$curanchor')==$anchor) //if mouse moves over same element again
				return false
			$loadarea_new.data('$curanchor', $anchor)
			if ($loadarea_new.data('$queueimage')){ //if a large image is in the queue to be shown
				$loadarea_new.data('$queueimage').unbind('load') //stop it first before showing current image
			}
			$loadarea_new.html($.thumbnailviewer2.loadmsg)
			...
			if ($hiddenimage.length==0){ //if this is the first time moving over or clicking on the anchor link
				...
				$hiddenimage.bind('loadevt', function(e){ //when enlarged image has fully loaded
					...
					$loadarea_new.empty().append($targetimage) //show enlarged image
					$.thumbnailviewer2.showimage($targetimage, s)
				})
			$loadarea_new.data('$queueimage', $hiddenimage) //remember currently loading image as image being queued to load
			}
