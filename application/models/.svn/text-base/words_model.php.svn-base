...
class Words_model extends CI_Model {
    ...
    function __construct()
    {
        //parent::Model();
		parent::__construct();
		$this->tbl = $this->db->WORDS; 
		$this->tbl_translation = $this->db->WORD_TRANSLATIONS; 
    }
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                ...
                $ret_[$i_cnt]["word_md5"]=stripslashes(htmlspecialchars_decode($row->word_md5));
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  ...
												  trim(htmlspecialchars($info["word_md5"], ENT_QUOTES, 'utf-8')),
												  ($info["is_active"])
												 ));
    ...
    * @returns $i_new_id  on success and FALSE if failed 
    */
    public function add_translated_word_info($info)
    {
        try
        {
            $i_ret_=0; ////Returns false
            if(!empty($info))
            {
                $s_qry="Insert Into ".$this->tbl_translation." Set ";
                $s_qry.=" word_id=? ";
                $s_qry.=", lang_id=? ";
				$s_qry.=", word=? ";
                $s_qry.=", is_active=? ";
