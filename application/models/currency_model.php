        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				...
				$ret_[$i_cnt]["currency_id"]=stripslashes(htmlspecialchars_decode($row->currency_id));
                $ret_[$i_cnt]["currency_details"]=stripslashes(htmlspecialchars_decode($row->currency_details));
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $s_qry.=" currency_id=? ";
                $s_qry.=", currency_details=? ";
                $s_qry.=", is_active=? ";
                
                $this->db->query($s_qry,array(
												  trim(htmlspecialchars($info["currency_id"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["currency_details"], ENT_QUOTES, 'utf-8')),
												  ($info["is_active"])
												 ));
